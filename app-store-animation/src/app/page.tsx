		...
		>
			...
			<motion.div layoutId={`card-content-${card.title}`} className="card-content">
				<div className="card-text">
					<motion.h2 layoutId={`card-heading-${card.title}`} className="card-heading">
						Game of the day
					</motion.h2>
				</div>
				<motion.div
				...
				>
					...
					<div className="desc-wrapper">
						<motion.span layoutId={`card-game-title-${card.title}`} className="game-title">
							{card.title}
						</motion.span>
						<motion.span layoutId={`card-game-subtitle-${card.title}`} className="game-subtitle">
							{card.description}
						</motion.span>
					</div>
					<motion.button layoutId={`card-button-${card.title}`} className="get-button">
						Get
					</motion.button>
				</motion.div>
			</motion.div>
		...
		>
			<div className="card-inner">
				...
				>
					...
					>
						...
						/>
						<div className="desc-wrapper">
							<motion.span layoutId={`card-game-title-${activeCard.title}`} className="game-title">
								{activeCard.title}
							</motion.span>
							<motion.span
			...
			>
				...
				<p>
					<b>The never ending adventure </b>
					In this game set in a fairy tale world, players embark on a quest through mystical lands
					filled with enchanting forests and towering mountains. Players can explore the world,
					build their own viking
				</p>
			</motion.div>
		</motion.div>
	);
...
export default function StyledWithoutDrag() {
	...
	return (
		<div className="cards-wrapper">
			...
			<AnimatePresence>
				{activeCard ? <ActiveCard activeCard={activeCard} setActiveCard={setActiveCard} /> : null}
			</AnimatePresence>
		</div>
	);
}
